...
from smartadvisor_discovery.models import ControlSpec, ProbeResult
from smartadvisor_discovery.selectors import (
...
def _probe_control(
    ...
    matches = _matching_elements(descendants, spec.automation_id)
...
def _find_smartadvisor_window(backend: str, title_pattern: str) -> Any | None:
...
def probe_backend(
    ...
    title_pattern: str = SMARTADVISOR_TITLE_PATTERN,
) -> dict[str, object]:
    ...
    try:
        window = _find_smartadvisor_window(backend, title_pattern)
...
def scan_controls(
    ...
    """Return a PII-safe control discovery report."""
